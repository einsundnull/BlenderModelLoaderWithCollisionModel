...
public class ShadowMapRenderer implements GLSurfaceView.Renderer {
    ...
    private static final String SHADOW_FRAGMENT_SHADER_CODE =
            "#version 300 es\n" +
                    "precision highp float;\n" +
                    "in vec4 v_ShadowSpacePos;\n" + // Use the output variable from the vertex shader
    ...
    private static final String SCENE_FRAGMENT_SHADER_CODE =
            "#version 300 es\n" +
                    "precision highp float;\n" +
                    "in vec3 v_LightDir;\n" +
                    "in vec3 v_FragPos;\n" +
    ...
    public ShadowMapRenderer(Context context, MainActivity activityMain, ArrayList<Object> objects, int screenWidth, int screenHeight) {
    ...
    private void drawScene(int program, float[] mvpMatrix, float[] viewMatrix, float[] projectionMatrix) {
        ...
        synchronized (objects) {
            for (Object object : objects) {
                object.draw(program, mvpMatrix, viewMatrix, projectionMatrix, gles20);
            }
        }
    ...
    public synchronized void speedUpSimulation() {
        synchronized (objects) {
            for (Object objectSphere : objects) {
